...
The package SHALL provide token-driven chart primitives including `Sparkline`, `BarChart`, `LineChart`, and `DonutChart` with accessible labels and data-driven rendering suitable for dashboard summaries.
...
The package SHALL provide composable KPI card primitives including header, title, value, trend, description, chart slot, and footer parts for dashboard metric displays.
...
The package SHALL provide token-driven layout primitives including `Box`, `Stack`, `Row`, `Container`, `Spacer`, and `Divider` for common application composition patterns.
