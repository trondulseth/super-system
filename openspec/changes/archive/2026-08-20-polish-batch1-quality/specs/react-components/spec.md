...
The package SHALL provide a `Label` supporting native label attributes, forwarded refs, optional required indication, and disabled styling. When a label wraps a single associated control and `required` or `disabled` is set, the label SHALL propagate the corresponding native accessibility attributes to that control.
...
- **GIVEN** the Batch 1 polish change is complete
